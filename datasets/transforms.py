...
def get_transforms(args):
    transforms = Transforms(image_set=args.image_set)
...
class Transforms:
    ...
    def __call__(self, img, target):
        ...
        new_img, new_target = self.transforms(img, {'boxes': boxes,
                                                    'labels': labels})
        item = {
            'image': new_img,
            'mask': torch.ones_like(new_img, dtype=torch.float32)[0],
            'bboxes': new_target['boxes'],
...
def collate_fn(batch):
    ...
    # image and mask padding for batch forwarding.
    max_size = _max_by_axis([list(img.shape) for img in image_lst])

    batch_shape = [len(batch)] + max_size
    b, c, h, w = batch_shape
    dtype = image_lst[0].dtype

    tensor = torch.zeros(batch_shape, dtype=dtype)
    mask = torch.zeros((b, h, w), dtype=torch.bool)
    for img, pad_img, m in zip(image_lst, tensor, mask):
...
def resize(image, target, size, max_size=None):
    ...
    if "masks" in target:
        masks = torch.nn.functional.interpolate(target["masks"][None, ...], size)[0] > 0.5
        target["masks"] = masks
