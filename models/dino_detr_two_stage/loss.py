...
def get_dino_detr2_criterion(args, device):
...
class Criterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if num_boxes == 0:
            num_boxes = 1
        outputs, (dn_outputs, (dn_targets, dn_indices)) = outputs
        dn_num_boxes = sum(len(t['labels']) for t in dn_targets)
        if dn_num_boxes == 0:
            dn_num_boxes = 1

        if not self.aux_flag:
            outputs = outputs[-1:]
            dn_outputs = dn_outputs[-1:]

        losses = defaultdict(list)
        # calculate losses per each decoder layer
        for output, dn_output in zip(outputs, dn_outputs):
            ...
            # dn loss
            dn_l1_loss, dn_giou_loss = self.calc_box_loss(dn_output, dn_targets, dn_indices, dn_num_boxes)
            losses['l1_loss'].append(dn_l1_loss)
            losses['giou_loss'].append(dn_giou_loss)

            dn_cls_loss = self.calc_cls_loss(dn_output, dn_targets, dn_indices, dn_num_boxes)
            losses['cls_loss'].append(dn_cls_loss)
        return losses
